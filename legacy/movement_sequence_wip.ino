...
Vector posiciones[]={{20.,20.},{23.5, 0}};
...
void loop() {
  if ((motorY.distanceToGo() > 0) && ( steps_m1 > 0 || (motorX.distanceToGo() == 0) || (steps_m2 < 0 && steps_m1 > steps_m2))) {
    motorY.run();
  } else {
    motorX.run();
  }
  ...
  if (motorY.distanceToGo() == 0 && motorX.distanceToGo() == 0) {
    if (idx%2 == 0) {
      update_steps(23.5, 20, &steps_m1, &steps_m2);
    } else {
      update_steps(23.5, 0, &steps_m1, &steps_m2);
    }
    idx++;
